...
int32_t MessageBuffer::write_uint16s(const uint16_t* data, uint32_t n) {
	...
	if (bufferHasMyEndianness) {
		...
		for (i=0; i<n; ++i) {
			an_uint16 = MESBUF_UINT16_SWAP_BYTES(data[i]);
...
int32_t MessageBuffer::read_uint16s(uint16_t* data, uint32_t n) {
	...
	if (bufferHasMyEndianness) {
		...
		for (i=0; i<n; ++i) {
			...
			data[i] = MESBUF_UINT16_SWAP_BYTES(an_uint16);
			readOffset += 2;
		}
	}
...
int32_t MessageBuffer::write_uint32s(const uint32_t* data, uint32_t n) {
	...
	if (bufferHasMyEndianness) {
		...
		for (i=0; i<n; ++i) {
			an_uint32 = MESBUF_UINT32_SWAP_BYTES(data[i]);
...
int32_t MessageBuffer::read_uint32s(uint32_t* data, uint32_t n) {
	...
	if (bufferHasMyEndianness) {
		...
		for (i=0; i<n; ++i) {
			...
			data[i] = MESBUF_UINT32_SWAP_BYTES(an_uint32);
			readOffset += 4;
		}
	}
...
int32_t MessageBuffer::write_uint64s(const uint64_t* data, uint32_t n) {
	...
	if (bufferHasMyEndianness) {
		...
		for (i=0; i<n; ++i) {
			...
			an_uint32 = MESBUF_UINT32_SWAP_BYTES(a_deux32.ui32[1]);
			memcpy(buffer+writeOffset,&an_uint32,4);
			writeOffset += 4;
			an_uint32 = MESBUF_UINT32_SWAP_BYTES(a_deux32.ui32[0]);
			memcpy(buffer+writeOffset,&an_uint32,4);
			writeOffset += 4;
		}
	}
...
int32_t MessageBuffer::read_uint64s(uint64_t* data, uint32_t n) {
	...
	if (bufferHasMyEndianness) {
		...
		for (i=0; i<n; ++i) {
			memcpy(&an_uint32,buffer+readOffset,4);
			a_deux32.ui32[1] = MESBUF_UINT32_SWAP_BYTES(an_uint32);
			readOffset += 4;

			memcpy(&an_uint32,buffer+readOffset,4);
			a_deux32.ui32[0] = MESBUF_UINT32_SWAP_BYTES(an_uint32);
			readOffset += 4;
